...
/*
   ...
   for (it = playersWithSpeed.end(); it != playersWithSpeed.begin(); ++it)
   {
       Player *p = it->second;
       orderOfPlay.push_back(p);
   }*/
...
void Match::rebound()
{
    ...
    do{
        ...
        if(count == 1)
        {
             ball.setPlayerPosition(players[0]->getNumber());
        ...
        else if(count > 1)
        {
             ...
             Player *p = players[result];
             if(p->getTeam() == 1)
             {
                int pos = teamOne->getPlayerPosition(p->getNumber());
             }
             else
             {
                 int pos = teamOne->getPlayerPosition(p->getNumber());
